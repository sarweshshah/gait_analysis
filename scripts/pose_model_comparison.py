...
def compare_models(video_path: str, output_dir: str = "model_comparison"):
    ...
    with UnifiedPoseProcessor(
        model_type="mediapipe", output_dir=output_dir  # Default, will be overridden
    ) as processor:
...
def process_with_model(video_path: str, model_type: str, output_dir: str = None):
    ...
    # Process video
    with UnifiedPoseProcessor(model_type=model_type, output_dir=output_dir) as processor:
...
def switch_models_demo(video_path: str, output_dir: str = "model_switching_demo"):
    ...
    with UnifiedPoseProcessor(model_type=available_models[0], output_dir=output_dir) as processor:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Compare and switch between pose estimation models")

    parser.add_argument("--video", "-v", required=True, help="Path to video file to process")
    parser.add_argument(
        "--model", "-m", choices=["mediapipe"], default="mediapipe", help="Pose model to use (default: mediapipe)"
    )
    ...
    parser.add_argument(
        "--output", "-o", default="outputs/mediapipe", help="Output directory (default: outputs/mediapipe)"
    )
