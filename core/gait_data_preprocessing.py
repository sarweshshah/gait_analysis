            ...
            for coord_idx in range(2):  # Only interpolate x, y coordinates
                ...
                if len(valid_times) > 3:
                    ...
                    spline = interpolate.interp1d(
                        valid_times, valid_values, kind="cubic", bounds_error=False, fill_value="extrapolate"
                    )
                else:
                    # Use linear interpolation for few points
                    spline = interpolate.interp1d(
                        valid_times, valid_values, kind="linear", bounds_error=False, fill_value="extrapolate"
                    )
