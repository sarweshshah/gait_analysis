...
class GaitTrainer:
    ...
    def create_model(self, input_shape: Tuple[int, int]) -> keras.Model:
        ...
        Returns:
            Compiled model
        """
        model = create_gait_tcn_model(input_shape=input_shape, task_type=self.task_type, **self.model_config)

        model = compile_gait_model(
            model=model, task_type=self.task_type, learning_rate=self.model_config.get("learning_rate", 0.001)
        )
    ...
    def train_with_cross_validation(
        ...
        **kwargs,
        ...
        cv_results = {"fold_scores": [], "fold_histories": [], "best_models": [], "predictions": [], "true_labels": []}
        ...
        for fold, (train_idx, test_idx) in enumerate(kfold.split(features, labels)):
            ...
            # Callbacks
            callbacks = self._create_callbacks(fold=fold, early_stopping_patience=early_stopping_patience, **kwargs)
            ...
            # Store results
            cv_results["fold_scores"].append({"fold": fold + 1, "test_loss": test_loss, "test_accuracy": test_accuracy})
    ...
    def evaluate_model_performance(self, cv_results: Dict):
        ...
        for score in cv_results["fold_scores"]:
            logger.info(
                f"Fold {score['fold']}: Accuracy = {score['test_accuracy']:.4f}, Loss = {score['test_loss']:.4f}"
            )
