...
def getKeypoints(prob_map, thres=0.1):
    ...
    mapMask = np.uint8(mapSmooth > thres)
    keyPoints = []
    ...
    for cnt in contours:
        ...
        keyPoints.append(maxLoc + (prob_map[maxLoc[1], maxLoc[0]],))

    return keyPoints


# Find valid connections between the different joints of a all persons present
def getValidPairs(output_image):
    validPairs = []
    invalidPairs = []
    n_interp_samples = 10
    paf_score_th = 0.1
    ...
    for k in range(len(mapIdx)):
        ...
        pafA = output_image[0, mapIdx[k][0], :, :]
        pafB = output_image[0, mapIdx[k][1], :, :]
