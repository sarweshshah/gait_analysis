...
class BasicGaitEvents:
    ...
    def detect_events(
        self, keypoints_sequence: np.ndarray, confidences: Optional[np.ndarray] = None
    ) -> Dict[str, List]:
        """
        ...
        logger.info(
            f"Detected {len(all_events['heel_strikes'])} heel strikes, " f"{len(all_events['toe_offs'])} toe offs"
        )
    ...
    def _detect_phases(
        self, heel_strikes: List[int], toe_offs: List[int], sequence_length: int
    ...
    def _detect_support_phases(self, events: Dict) -> Dict:
        ...
        for hs in events["heel_strikes"]:
            ...
            nearby_toe_offs = [
                to
                for to in events["toe_offs"]
                if to["side"] == opposite_side and abs(to["frame"] - hs["frame"]) < int(0.2 * self.fps)
            ]
    ...
    def calculate_gait_metrics(self, events: Dict) -> Dict[str, float]:
        ...
        if len(left_heel_strikes) > 1:
            left_stride_times = [
                left_heel_strikes[i + 1]["time"] - left_heel_strikes[i]["time"]
        ...
        if len(right_heel_strikes) > 1:
            right_stride_times = [
                right_heel_strikes[i + 1]["time"] - right_heel_strikes[i]["time"]
