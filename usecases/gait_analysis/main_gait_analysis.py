...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("outputs/logs/gait_analysis.log"), logging.StreamHandler(sys.stdout)],
)
...
class GaitAnalysisPipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Features enabled: Pose Detection=Always, "
    ...
    def setup_pose_processor(self):
        ...
        self.pose_processor = UnifiedPoseProcessor(model_type=pose_model, **processor_kwargs)
        ...
        for dir_path in json_dirs:
            try:
                processed = self.data_preprocessor.process_video_sequence(
                    json_directory=dir_path, fps=self.config.get("fps", 30.0)
                )
                ...
                results["videos"].append({"json_dir": dir_path, "error": str(e)})
            ...
            if self.enable_gait_analysis:
                ...
                if video_paths:
                    ...
                    self.visualizer.process_video(
                        video_path=video_paths[0], loop_video=self.config.get("loop_video", False)
                    )
...
def main():
    ...
    parser.add_argument(
        "--task", choices=["phase_detection", "event_detection"], default="phase_detection", help="Analysis task type"
    )
    ...
    parser.add_argument(
        "--enable-gait-analysis", action="store_true", default=True, help="Enable gait analysis feature"
    )
...
GaitAnalysisMain = GaitAnalysisPipeline
