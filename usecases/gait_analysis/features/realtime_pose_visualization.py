...
class RealTimePoseVisualizer:
    ...
    def _estimate_from_anatomy(
        self, joint_idx: int, keypoints: List[Tuple[int, int, float]]
        ...
        if (
            ref1_idx < len(keypoints)
            and ref2_idx < len(keypoints)
            and keypoints[ref1_idx][2] > 0.5
            and keypoints[ref2_idx][2] > 0.5
        ):
    ...
    def _draw_keypoints(
        self, frame: np.ndarray, keypoints: List[Tuple[int, int, float]], show_confidence: bool = False
    ):
        ...
        for i, (x, y, conf) in enumerate(keypoints):
            if conf > 0.3:  # Draw keypoints with some confidence (lowered for estimated points)
                ...
                if show_confidence:
                    cv2.putText(frame, f"{conf:.2f}", (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
    ...
    def _draw_connections(self, frame: np.ndarray, keypoints: List[Tuple[int, int, float]]):
        ...
        for connection in self.connections:
            if len(connection) == 2:
                ...
                if (
                    idx1 < len(keypoints)
                    and idx2 < len(keypoints)
                    and keypoints[idx1][2] > 0.5
                    and keypoints[idx2][2] > 0.5
                ):
    ...
    def _draw_info(
        ...
        # Add border
        cv2.rectangle(frame, (panel_x, panel_y), (panel_x + panel_width, panel_y + panel_height), (60, 60, 60), 1)
        ...
        statuses = {"Trail": trail_enabled, "Connections": connections_enabled, "Confidence": show_confidence}
...
def main():
    ...
    visualizer = RealTimePoseVisualizer(
        model_complexity=args.model_complexity, min_detection_confidence=args.min_confidence
    )
