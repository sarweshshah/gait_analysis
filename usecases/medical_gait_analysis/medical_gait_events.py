...
class MedicalGaitEvents(ABC):
    ...
    def calculate_temporal_parameters(self, events: Dict[str, List[int]]) -> Dict[str, float]:
        ...
        if "heel_strike_right" in events and len(events["heel_strike_right"]) > 1:
            ...
            stride_times = [
                (heel_strikes[i + 1] - heel_strikes[i]) * self.frame_duration for i in range(len(heel_strikes) - 1)
            ]
        ...
        if "toe_off_right" in events and "heel_strike_right" in events:
            stance_durations = self._calculate_phase_durations(events["heel_strike_right"], events["toe_off_right"])
    ...
    def _calculate_phase_durations(
        self, start_events: List[int], end_events: List[int], next_cycle: bool = False
    ) -> List[int]:
...
class HydrocephalusGaitEvents(MedicalGaitEvents):
    ...
    def _detect_heel_strikes(self, ankle_positions: np.ndarray) -> List[int]:
        ...
        for i in range(1, len(y_positions) - 1):
            if y_positions[i] < y_positions[i - 1] and y_positions[i] < y_positions[i + 1] and y_velocity[i] < 0:
    ...
    def _detect_toe_offs(self, toe_positions: np.ndarray, ankle_positions: np.ndarray) -> List[int]:
        ...
        for i in range(1, len(toe_velocity) - 1):
            if (
                toe_velocity[i] > 0.01  # Significant upward velocity
                and toe_velocity[i] > toe_velocity[i - 1]
                and toe_velocity[i] > toe_velocity[i + 1]
            ):
                toe_offs.append(i)
    ...
    def calculate_hydrocephalus_metrics(
        self, events: Dict[str, List[int]], keypoints_sequence: np.ndarray
    ) -> Dict[str, float]:
        """
        ...
        if "freezing_episodes" in events:
            ...
            if events["freezing_episodes"]:
                freezing_durations = [
                    (end - start + 1) * self.frame_duration for start, end in events["freezing_episodes"]
                ]
